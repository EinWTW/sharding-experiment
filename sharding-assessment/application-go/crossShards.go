...
func main() {
	...
	log.Println("--> Evaluate Transaction: ReadAccount")
	result, err := contract1.EvaluateTransaction("ReadAccount", account1)
	if err != nil {
		log.Fatalf("Failed to evaluate transaction: %v", err)
	}
	log.Println(string(result))

	result, err = contract2.EvaluateTransaction("ReadAccount", account2)
	if err != nil {
		log.Fatalf("Failed to evaluate transaction: %v", err)
	}
	log.Println(string(result))
